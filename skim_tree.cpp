...
int main(int argc, char ** argv){
	if (argc != 5){
		cerr << "Wrong number of arguments. Instead try\n"
         << "\tskim_tree /path/to/input/file /path/to/output/file 000000000 00000\n\n";
    ...
    cout << "where the fourth argument must be an integer of 5 digits" << endl;
    cout << "1st digit: fiducial cut status electrons       0 = off, 1 = on" << endl;
    cout << "2nd digit: fiducial cut status protons         0 = off, 1 = on" << endl;
    cout << "3rd digit: fiducial cut status neutrons        0 = off, 1 = on" << endl;
    cout << "4th digit: fiducial cut status pion+           0 = off, 1 = on" << endl;
    cout << "5th digit: fiducial cut status pion-           0 = off, 1 = on" << endl;

    return -1;
	}

	// --------------------------------------------------------------------------------------------------
	// Specifying reaction for output file
  ...
  if(strlen(argv[4]) != 5){
		cout << endl << "Fourth argument must be an integer of 5 digits." << endl;
		cout << "You have specified an integer of " << strlen(argv[4]) << " digits" << endl;
		exit(1);
	}
  ...
  const bool do_fid_e = atoi(argv[4])/10000 & 1;
  const bool do_fid_p = atoi(argv[4])/1000 & 1;
  const bool do_fid_n = atoi(argv[4])/100 & 1;
  const bool do_fid_pip = atoi(argv[4])/10 & 1;
  const bool do_fid_pim = atoi(argv[4]) & 1;
                          ...
                          )
            {
              ...
              if( Beta_corr < 0.95 )
                ...
                // Instead see if ECal hit is neutron like.
                {
                  ...
                  if ((do_fid_n) && (!(fid_params.CutUVW( n_ec_xyz ,10.))))
                    continue;
